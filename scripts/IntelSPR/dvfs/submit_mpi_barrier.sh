...
#SBATCH --exclusive
#SBATCH --time=2:00:00
#SBATCH -o ./out/mpi_barrier.%j.out
#SBATCH -e /dev/null
...
module load likwid/5.4.1-perf openmpi/5.0.8
...
srun --cpu-bind=cores --distribution=block:block \
  bash -c '
    if [ "$SLURM_PROCID" -eq 0 ]; then
      exec taskset -c 0 likwid-perfctr -f -c 0 -g HPC_DVFS_MODEL_INTEL -t 500ms -O -- "'"${BIN}/mpi_barrier"'"
    else
      exec "'"${BIN}/mpi_barrier"'" >/dev/null 2>&1
    fi
  ' \
  2> "${RESULT_DIR}/mpi_barrier.${FREQ_MHZ}.${SLURM_JOB_ID}.prof"