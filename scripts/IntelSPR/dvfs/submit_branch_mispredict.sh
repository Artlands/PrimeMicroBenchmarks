...
#SBATCH --exclusive
#SBATCH --time=2:00:00
#SBATCH -o ./out/branch_mispredict.%j.out
#SBATCH -e /dev/null
...
srun --cpu-bind=cores --distribution=block:block \
  bash -c '
    if [ "$SLURM_PROCID" -eq 0 ]; then
      exec taskset -c 0 likwid-perfctr -f -c 0 -g HPC_DVFS_MODEL_INTEL -t 500ms -O -- "'"${BIN}/branch_mispredict"'"
    else
      exec "'"${BIN}/branch_mispredict"'" >/dev/null 2>&1
    fi
  ' \
  2> "${RESULT_DIR}/branch_mispredict.${FREQ_MHZ}.${SLURM_JOB_ID}.prof"
