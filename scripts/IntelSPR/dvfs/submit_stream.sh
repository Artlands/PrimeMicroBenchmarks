...
srun --cpu-bind=cores --distribution=block:block \
  bash -c '
    if [ "$SLURM_PROCID" -eq 0 ]; then
      exec taskset -c 0 likwid-perfctr -f -c 0 -g HPC_DVFS_MODEL_INTEL -t 500ms -O -- "'"${BIN}/stream"'"
    else
      exec "'"${BIN}/stream"'" >/dev/null 2>&1
    fi
  ' \
  2> "${RESULT_DIR}/stream.${FREQ_MHZ}.${SLURM_JOB_ID}.prof"