...
srun --cpu-bind=cores --distribution=block:block \
  bash -c '
    if [ "$SLURM_PROCID" -eq 0 ]; then
      exec taskset -c 0 likwid-perfctr -f -c 0 -g HPC_DVFS_MODEL_INTEL -t 500ms -O -- "'"${BIN}/tree_walk"'"
    else
      exec "'"${BIN}/tree_walk"'" >/dev/null 2>&1
    fi
  ' \
  2> "${RESULT_DIR}/tree_walk.${FREQ_MHZ}.${SLURM_JOB_ID}.prof"