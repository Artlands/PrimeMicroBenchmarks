...
#SBATCH --exclusive
#SBATCH --time=4:00:00
#SBATCH -o 644.nab_s/speccpu_nab.%A.out
#SBATCH -e 644.nab_s/speccpu_nab.%A.err
...
cd "${SPECHPC_DIR}"
source shrc
go "644.nab_s" "${RUN_SUBDIR}"

echo "Launching 644.nab with 256 MPI instances"

likwid-perfctr -f -c 0,128 -g ENERGY -t 500ms -O \
  -- srun --mpi=pmix --cpu-bind=cores --distribution=block:block ./nab_s 3j1n 20140317 220 \
  2> "${RESULT_DIR}/speccpu_nab.${SLURM_JOB_ID}.prof"
